...
module Memory(clk, reset_n, i_readM, i_writeM, i_address, i_data, d_readM, d_writeM, d_address, d_data);
    ...
    assign target_address = {d_address[15:2],2'b0};
    //load process
        ...
        if(!reset_n) begin
            ...
            if(d_writeM && ls_counter == 3'd2) begin
                case(d_address[1:0])
                    ...
                    2'b10 : begin
                        memory[d_address] <= d_data[2*`WORD_SIZE-1:`WORD_SIZE];
                    end
                    2'b01 : begin
                        memory[d_address] <= d_data[3*`WORD_SIZE-1:2*`WORD_SIZE];
                    end
                    2'b00 : begin
                        memory[d_address] <= d_data[4*`WORD_SIZE-1:3*`WORD_SIZE];
                    end
                endcase
            end
        end
